...
Feeds into:
- model --> practice_test_RF_model.py (working OK)
- CSV --> practice_H_train_RF_model.py (to be tested)

This finally works!!!! To be cleaned up...
Running...

Updates since last push:
- This now outputs the correct CSV, with:
predictions, satellite data and L2 groundtruth for the same row
- Calculate min pixels per class in script, rather than requiring as command-line arg.
- Logging accuracy per hyperparm combination
...
## OLD WORKING CODE: I want to use the same number of pixels for each class (oversampling/undersampling), to create a balanced model
#min_pixels_per_class = args.pixels_per_class

### Change min_pixels_per_class to something like the following:
...
## Record indices of testing dataset only
...
## Save DF with predictions, satellite data and L2 ground truth to CSV file
